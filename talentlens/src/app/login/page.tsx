  ...
  return (
    <motion.div
      ...
      style={{ left: `${x}%`, top: `${y}%`, width: size, height: size,
               background: 'rgba(99,179,237,0.35)', filter: 'blur(0.5px)' }}
      animate={{ y: [0, -18, 0], opacity: [0.15, 0.55, 0.15], scale: [1, 1.3, 1] }}
  ...
  return (
    <span className={`inline-flex items-end ${cls}`} aria-label="Aptio">
      ...
      <motion.span
        ...
        transition={{ duration: 1.6, delay: 1.4, repeat: Infinity, repeatDelay: 4, ease: 'easeInOut' }}
      />
    </span>
  );
}
...
function OrbitDot({ radius, angle, delay }: { radius: number; angle: number; delay: number }) {
  return (
    <motion.div
      ...
      animate={{
        x: [
          Math.cos(angle) * radius, Math.cos(angle + Math.PI) * radius, Math.cos(angle + 2 * Math.PI) * radius,
        ],
        y: [
          Math.sin(angle) * radius, Math.sin(angle + Math.PI) * radius, Math.sin(angle + 2 * Math.PI) * radius,
        ],
        opacity: [0.8, 0.2, 0.8],
      }}
...
export default function LoginPage() {
  ...
  // Particles: generate on client only to avoid hydration mismatch
  ...
  return (
    <div className="min-h-screen flex overflow-hidden"
      ...
      <div className="hidden lg:flex flex-col justify-between w-[520px] shrink-0 p-12 relative overflow-hidden"
          ...
          <div className="space-y-3">
            {FEATURES.map((f, i) => (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: 1.2 + i * 0.12, ease: [0.22, 1, 0.36, 1] }}
        ...
        >
          {/* Floating animation on the quote card */}
          <motion.div
            animate={{ y: [0, -4, 0] }}
          ...
          <div className="rounded-2xl p-8"
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="w-full py-3 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-50 relative overflow-hidden"

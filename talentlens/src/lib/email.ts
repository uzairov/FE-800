...
export async function sendFeedback360Invite(params: {
  to: string;
  reviewName: string;
  rateUrl: string;
}): Promise<void> {
  const { to, reviewName, rateUrl } = params;
  const apiKey = process.env.RESEND_API_KEY;
  const from   = process.env.EMAIL_FROM ?? 'Aptio <noreply@aptio.app>';

  if (!apiKey || apiKey === 'your_resend_key_here') {
    console.log('[email:DEV] 360 invite to:', to, 'url:', rateUrl);
    return;
  }
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from, to: [to],
      subject: `Приглашение оценить коллег — ${reviewName}`,
      html: `<div style="font-family:sans-serif;max-width:480px;margin:40px auto;padding:32px;background:#111827;border-radius:16px;color:#e5e7eb">
        <div style="display:inline-flex;align-items:center;gap:10px;margin-bottom:24px">
          <div style="width:36px;height:36px;background:#2563eb;border-radius:10px;display:inline-flex;align-items:center;justify-content:center">
            <span style="color:#fff;font-weight:900;font-size:16px">A</span>
        ...
        <h2 style="color:#f9fafb;margin:0 0 12px">Оценка 360°: ${reviewName}</h2>
        <p style="color:#9ca3af;line-height:1.6;margin:0 0 24px">
          Вас приглашают принять участие в 360° оценке коллег. Это займёт 5–10 минут. Ваши ответы анонимны.
        </p>
        <a href="${rateUrl}" style="display:inline-block;background:#2563eb;color:#fff;padding:13px 30px;border-radius:10px;text-decoration:none;font-weight:600;font-size:15px">Оценить коллег →</a>
        <p style="color:#4b5563;font-size:12px;margin-top:28px">Ссылка одноразовая. Не передавайте её третьим лицам.</p>
      </div>`,
    }),
  });
}
